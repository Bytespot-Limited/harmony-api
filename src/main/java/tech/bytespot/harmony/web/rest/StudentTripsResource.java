...
public class StudentTripsResource {
   ...
   *     studentTripsDTO, or with status {@code 400 (Bad Request)} if the studentTrips has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/student-trips")
  public ResponseEntity<StudentTripsDTO> createStudentTrips(
      @Valid @RequestBody StudentTripsDTO studentTripsDTO) throws URISyntaxException {
    log.debug("REST request to save StudentTrips : {}", studentTripsDTO);
    if (studentTripsDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new studentTrips cannot already have an ID", ENTITY_NAME, "idexists");
    }
    StudentTripsDTO result = studentTripsService.save(studentTripsDTO);
    return ResponseEntity.created(new URI("/api/student-trips/" + result.getId()))
        .headers(
   ...
   *     studentTripsDTO, or with status {@code 400 (Bad Request)} if the studentTripsDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the studentTripsDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, false, ENTITY_NAME, studentTripsDTO.getId().toString()))
        .body(result);
  }

  /**
   * {@code PATCH /student-trips/:id} : Partial updates given fields of an existing studentTrips,
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated
   *     studentTripsDTO, or with status {@code 400 (Bad Request)} if the studentTripsDTO is not
   *     valid, or with status {@code 404 (Not Found)} if the studentTripsDTO is not found, or with
   *     status {@code 500 (Internal Server Error)} if the studentTripsDTO couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PatchMapping(
      value = "/student-trips/{id}",
  ...
  public ResponseEntity<StudentTripsDTO> partialUpdateStudentTrips(
      ...
      throws URISyntaxException {
    log.debug(
        "REST request to partial update StudentTrips partially : {}, {}", id, studentTripsDTO);
    if (studentTripsDTO.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    if (!Objects.equals(id, studentTripsDTO.getId())) {
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of studentTrips in
   *     body.
   */
   ...
   *     studentTripsDTO, or with status {@code 404 (Not Found)}.
   */
